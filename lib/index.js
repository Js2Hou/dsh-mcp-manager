...
function unresolvedJsExprPaths(value, path = "", out = []) {
  if (isJsExpr(value)) {
...
function probeConfigFor(entry) {
  ...
  if (pending.length > 0) {
    return {
      ok: false,
      reason: `This server is not running, so its !!js expressions have not been evaluated yet (${pending.join(", ")}). Enable it, then probe again.`
    };
  }
...
async function dispatch(ctx, patchFile, endpoint, payload) {
  switch (endpoint) {
    ...
    case "probe": {
      ...
      const result = await probeServer(source.config);
      return ok(result);
    }
    default:
